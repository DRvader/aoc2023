...
fn parse_race(i: &str) -> IResult<&str, (f64, f64)> {
    let (i, time) = tuple((
        multispace0,
        tag("Time:"),
        space1,
        many1(terminated(digit1, multispace1)),
    ))(i)
    .map(|(i, (_, _, _, out))| (i, out.join("").replace(" ", "").parse().unwrap()))?;
    let (i, distance) = tuple((
        tag("Distance:"),
        space1,
        many1(terminated(digit1, multispace1)),
    ))(i)
    .map(|(i, (_, _, out))| (i, out.join("").replace(" ", "").parse().unwrap()))?;
...
pub fn main() {
    let (_, (time, distance)) = all_consuming(parse_race)(INPUT).finish().unwrap();
    ...
    let number_of_ways_to_win = (b - a) as usize + 1;

    println!("I can win the race in {number_of_ways_to_win}");
}
