...
pub fn main() {
    ...
    let mut sum = 0;
    for game in games {
        let mut min_game = Turn::default();
        for turn in game.turns {
            if turn.red > min_game.red {
                min_game.red = turn.red;
            }

            if turn.green > min_game.green {
                min_game.green = turn.green;
            }
            if turn.blue > min_game.blue {
                min_game.blue = turn.blue;
            }
        }

        sum += min_game.red * min_game.green * min_game.blue;
    }
