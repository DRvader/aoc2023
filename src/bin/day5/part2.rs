...
fn parse_seed_conversion(i: &str) -> IResult<&str, usize> {
    let (i, (_, _, seeds, _)) = tuple((
        multispace0,
        tag("seeds: "),
        separated_list1(multispace1, tuple((digit1, space1, digit1))),
        multispace1,
    ))(i)?;
    let seeds = seeds
        .into_iter()
        .map(|v| (v.0.parse().unwrap(), v.2.parse().unwrap()))
    ...
    while let Some((target_list, (current_start, current_end))) = worklist.pop() {
        if target_list < maps.len() {
            let mut pushed = false;
            for value in &maps[target_list].2 {
                if (current_start >= value.source_start
                    && current_start < value.source_start + value.len)
                    || (current_end > value.source_start
                        && current_end <= value.source_start + value.len)
                {
                    let current_start = if current_start < value.source_start {
                        worklist.push((target_list, (current_start, value.source_start)));
                    ...
                    let current_end = if current_end > value.source_start + value.len {
                        worklist.push((target_list, (value.source_start + value.len, current_end)));

                        value.source_start + value.len
                    } else {
                        current_end
                    };
                    ...
                    worklist.push((
                        target_list + 1,
                        (
                            value.dest_start + (current_start - value.source_start),
                            value.dest_start + (current_end - value.source_start),
                        ),
                    ));
                }
            }
            ...
            if !pushed {
                worklist.push((target_list + 1, (current_start, current_end)));
            }
        } else if lowest_location > current_start {
            lowest_location = current_start;
        }
    }
